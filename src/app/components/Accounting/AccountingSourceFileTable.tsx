...
import { removeAccountingFile, updateAccountingFile } from '@/app/store/file/accountingFileActions';
...
interface AccountingSourceFileTableProps {
...
export default function AccountingSourceFileTable({
  sourceFiles,
  onPreview,
  onEdit,
  onDelete,
  loading,
}: AccountingSourceFileTableProps) {
  ...
  const handleDeleteFile = () => {
    if (fileToDeleteIndex !== null) {
      ...
      if (deletedFile && deletedFile?.id) {
        // ใช้ accounting action
        dispatch(removeAccountingFile(deletedFile.id));
  ...
  const handleSaveImage = async (editedFile: File) => {
    ...
    // ใช้ accounting action
    dispatch(updateAccountingFile(updatedFile));
    onEdit(id ?? 0);
  };
        ...
        <table className="table-auto w-full border border-gray-300 rounded-lg overflow-hidden shadow-sm">
          ...
          {loading ? (
            ...
            sourceFiles?.map((item, index) => {
              const uniqueKey = `accounting-${item.fileName}-${item.fileType}-${index}`;
              return (
                <tr
                  key={uniqueKey}
